...
class DataProcessor:
    ...
    def _create_records_with_timestamps(
        ...
        file_duration = len(audio) / SAMPLE_RATE * 1000
        idx = 0
        while idx < len(utterances):
            ...
            if (
                ...
                idx += 1
                continue
            if segment_start > file_duration:
                tqdm.write(
                    f"Skipping {audio_path} ({format_timestamp(segment_start / 1000)}-"
                    f"{format_timestamp(segment_end / 1000)}) because file length "
                    f"{file_duration / 1000} is exceeded."
                )
                break;              
    ...
    def _save_segment_audio(self, audio: torch.Tensor, segment_start: int, dump_dir: Path) -> str:
        ...
        torchaudio.save(segment_audio_path, segment_audio.unsqueeze(0), SAMPLE_RATE, bits_per_sample=16)
